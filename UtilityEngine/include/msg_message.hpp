...
public:
	...
	bool comfirm(net_size_t& size)
	{
		...
		net_size_t len;
		net_size_t last_readable = this->readable_size(0);
		do {
			// no new data
			if (last_readable == 0)
			{
			...
			do {
				...
				p = this->next(len);
				if (!p) break;
				size = static_cast<net_size_t>(strlen(p));
...
{
////////////////////////////////////////////////////////////////////////////////////////////////////
template<class buffer_type, net_size_t MAX_MSG_LEN,const char* Delimiter>
class message_wrap : public _message_impl<buffer_type, MAX_MSG_LEN>
{
public:
	using buffer_t = buffer_type;
